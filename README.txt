...
Для клонирования удаленного репозитария с помощью команды clone нет необходимости создавать отдельную папку, вызывается git bash прямо в корневом каталоге и в него клонируется уже удаленный репозитарий
...
----------------------------------------------------------------------------
git fetch origin - загрузить весь репозиторий на локальный диск, но не производить слияние с локальным. Origin - алиас удаленного репозитария, можно вставить URL путь удаленого репозитария
git merge local_branch origin/remote_branch - после команды fetch скачать в локальную ветку local_branch из удаленной remote_branch
----------------------------------------------------------------------------
...
git push origin :nameBranch или git push origin --delete nameBracnh - удалить удаленную ветку из репозитария, предварительно нужно перейти на другую удаленую ветку по умолчанию
