...
git clone [url] - команда для клонирования репозитария с удаленного сервера. При этом команда создает папку на локальном диске с именем
как у удаленного репозитария, иницилизирует каталог .git, скачивает все файлыс удаленного репозитория, созадет рабочую копию последней версии

git clont [url] newName - тоже самое, только создается локальный репозиторий с овым именем newName
...
git commit -a -m '' - в этой команде совмещены две команды 'add' И 'commit'
...
git push -u origin master - ключ '-u' позволяет 
...
git branch -d nameNewBranch - производит удаление ветки nameNewBranch, прежде чем ее получится удалить, нужно перейти на другую ветку
...
git remote - список удаленных серверов, куда подключен локальный репозитарий, имена серверов - orgin или может быть другое имя
